...
static rt_err_t rt_stm32_eth_init(rt_device_t dev)
{
    ...
    /* Call ETH_StructInit if you don't like to config`ure all ETH_InitStructure parameter */
...
void rt_hw_stm32_eth_init(void)
{
    ...
    rt_kprintf("MAC:%02x-%02x-%02x-%02x-%02x-%02x\n",
        stm32_eth_device.dev_addr[0],
        stm32_eth_device.dev_addr[1],
        stm32_eth_device.dev_addr[2],
        /* generate MAC addr from 96bit unique ID (only for test). */
        stm32_eth_device.dev_addr[3],
        stm32_eth_device.dev_addr[4],
        stm32_eth_device.dev_addr[5]
    );
